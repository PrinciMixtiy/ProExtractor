...
def check_dependencies():
    ...
    results = {
        'ffmpeg': ffmpeg_manager.is_available()
    }
    return results
...
def setup_logging():
    ...
    # Use addHandler() instead of basicConfig() so the file handler is
    # always registered, even if module-level imports already triggered
    # Python's "last resort" handler or otherwise pre-configured the root logger.
...
def main():
    # Support for multiprocessing in frozen executables
    ...
    if not deps['ffmpeg']:
        logging.error(
            "Mandatory dependency 'FFmpeg' not found. Shutting down.")
        msg = QMessageBox()
